...
<body>
<div style="margin-top: 39px; background: #f1f1f1; width: 100%; padding: 0;">
			...
			<div style="background: #FCFCFC;">
				<div class="w3-container w3-padding-16">
					<span class="w3-left">
						...
						<c:if test="${date_opt != null}">
						<a href="${pageContext.request.contextPath}/diary/diary_content?num=${diary.num}&email=${diary.user_email}&date_opt=${date_opt}&pageNum=${currentPage}&type=gallery" 
						class="w3-text-gray w3-hover-text-black font-nanum-coding-c" style="font-size: 12.5pt; text-decoration: none;">
							${diary.subject}
						</a>
						</c:if>					
			...
			<div class="w3-center font-montserrat-c" style="background: #FCFCFC; margin: 1.5% 5%; padding: 2px 4px !important;">
				...
				<form method="post" action="${pageContext.request.contextPath}/diary/diary_gallery">
					...
					<select class="w3-border" style="display: inline-block; font-size: 10pt;" name="date_opt">
						<option class="w3-gray w3-text-light-gray" selected disabled>Date Select</option>
		...
		<c:if test="${date_opt != null}">
			...
			<div class="w3-center font-montserrat-c" style="background: #FCFCFC; margin: 1.5% 5%; padding: 2px 4px !important;">
				...
				<form method="post" action="${pageContext.request.contextPath}/diary/diary_gallery">
					...
					<select class="w3-border" style="display: inline-block; font-size: 10pt;" name="date_opt">
						<option class="w3-gray w3-text-light-gray" disabled>Date Select</option>
						<option class="w3-light-gray" value= "${date_opt}" selected>${date_opt}</option>
						<c:forEach var="diary" items="${date_list}">
						<option value="${diary.d_date}">${diary.d_date}</option> 
						</c:forEach>
					</select> 			
				...
				<c:if test="${count>0}">
					<c:if test="${startPage > bottomLine}">
					<a href="diary_gallery?email=${s_email}&date_opt=${date_opt}&pageNum=${startPage - bottomLine}" style="font-size: 11pt;">[이전]</a>
					</c:if>
			
					<c:forEach var="i" begin="${startPage}" end="${endPage}">
					<a href="diary_gallery?email=${s_email}&date_opt=${date_opt}&pageNum=${i}"> 
						<c:if test="${i != currentPage}">
							[${i}]
						</c:if> 
						<c:if test="${i == currentPage}">
					...
					<c:if test="${endPage < pageCount}">
					<a href="diary_gallery?email=${s_email}&date_opt=${date_opt}&pageNum=${startPage + bottomLine}" style="font-size: 11pt;">[다음]</a>
					</c:if>
				</c:if>
			</div>
			</c:if>
